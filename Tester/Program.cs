...
var testData = GetTestingData(resolution, DateTime.UtcNow.AddDays(-5), DateTime.UtcNow.AddDays(-1));
var dataAccess = new DbAccess(consoleLogger);
dataAccess.SavePrices(Converter.ConvertBrocerPriceToDbPrice(testData, resolution, testEpic));
...
PricesEntity GetTestingData(Resolution resolution, DateTime startDate, DateTime endDate)
{
    ...
    if (prices.Prices.Any())
        while ((prices.Prices.Last().SnapshotTime.ToUniversalTime() - prices.Prices.First().SnapshotTime.ToUniversalTime()) < ApplicationConstants.ResolutionDateTimeMapping[resolution])
        {
